...
const AdminBase = ({ props, authUser }) => {
  ...
  const [admin, dispatchAdmin] = React.useReducer(adminReducer, {
  ...
  const onFeedDeleteOk = async () => {
    dispatchAdmin({ type: REDUCER_ACTIONS.FEED_DELETE_INIT });
    await Feed.callCloudFunctionDeleteFeeds({
      firebase: firebase,
      daysOffset: admin.deleteFeeds.dayOffset,
    })
      .then((message) => {
        dispatchAdmin({
      ...
      <Container className={classes.container} component="main" maxWidth="sm">
        <Grid container spacing={2}>
          <Grid item xs={12} sm={6}>
            <PanelDeleteFeeds
              deleteFeeds={admin.deleteFeeds}
              onChangeField={onChangeDeleteFeedField}
              onOpenPopUp={onFeedDeletePopUpOpen}
            />
          </Grid>
        </Grid>
        {/* NEXT_FEATURE: Admin Bereich in Tabs gliedern */}
        {/* https://material-ui.com/components/tabs/ */}
        {/* <Typography color="error">Systemmeldungen</Typography>
        <Typography color="error">Default-Werte?</Typography>
        <Typography color="error">Produkte Mergen</Typography> */}
      </Container>
